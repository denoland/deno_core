...
#[op2]
pub fn op_call_promise_resolver<'s>(scope: &'s mut v8::HandleScope, f: &v8::Function) {
  let recv = v8::undefined(scope).try_into().unwrap();
  f.call(scope, recv, &[]);
}

#[op2]
pub fn op_resolve_promise<'s>(scope: &'s mut v8::HandleScope) -> v8::Local<'s, v8::Promise> {
