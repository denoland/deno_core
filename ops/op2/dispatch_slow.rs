...
pub(crate) fn generate_dispatch_slow(
  ...
  Ok(
    gs_quote!(generator_state(opctx, info, slow_function, slow_function_metrics) => {
      extern "C" fn #slow_function(#info: *const __deno_core::v8::FunctionCallbackInfo) {
        #with_scope
        #with_retval
        #with_args
        #with_opctx
      ...
      extern "C" fn #slow_function_metrics(#info: *const __deno_core::v8::FunctionCallbackInfo) {
        let args = __deno_core::v8::FunctionCallbackArguments::from_function_callback_info(unsafe {
          &*info
        });
        let #opctx = unsafe {
            &*(__deno_core::v8::Local::<__deno_core::v8::External>::cast(args.data()).value()
                as *const __deno_core::_ops::OpCtx)
        };

        __deno_core::_ops::dispatch_metrics_slow(&#opctx, __deno_core::_ops::OpMetricsEvent::Dispatched);
        Self::#slow_function(#info);
        __deno_core::_ops::dispatch_metrics_slow(&#opctx, __deno_core::_ops::OpMetricsEvent::Completed);
      }
    }),
  )
}
...
pub(crate) fn with_retval(generator_state: &mut GeneratorState) -> TokenStream {
  gs_quote!(generator_state(retval, info) =>
    (let mut #retval = __deno_core::v8::ReturnValue::from_function_callback_info(unsafe { &*#info });)
  )
}
...
pub(crate) fn with_fn_args(
  ...
  gs_quote!(generator_state(info, fn_args) =>
    (let #fn_args = __deno_core::v8::FunctionCallbackArguments::from_function_callback_info(unsafe { &*#info });)
  )
}
...
pub(crate) fn with_opctx(generator_state: &mut GeneratorState) -> TokenStream {
  ...
  gs_quote!(generator_state(opctx, fn_args) =>
    (let #opctx = unsafe {
    &*(__deno_core::v8::Local::<__deno_core::v8::External>::cast(#fn_args.data()).value()
        as *const __deno_core::_ops::OpCtx)
    };)
  )
}
...
pub fn from_arg(
  ...
  let res = match arg {
      ...
      *needs_isolate = true;
      quote! {
        let #arg_ident = if #arg_ident.is_null_or_undefined() {
          ...
          Some(__deno_core::_ops::to_string(&mut #scope, &#arg_ident))
        };
      }
    }
    Arg::String(Strings::String) => {
      ...
      *needs_isolate = true;
      quote! {
        let #arg_ident = __deno_core::_ops::to_string(&mut #scope, &#arg_ident);
      }
    }
    Arg::String(Strings::RefStr) => {
      ...
      *needs_isolate = true;
      quote! {
        ...
        let mut #arg_temp: [::std::mem::MaybeUninit<u8>; __deno_core::_ops::STRING_STACK_BUFFER_SIZE] = [::std::mem::MaybeUninit::uninit(); __deno_core::_ops::STRING_STACK_BUFFER_SIZE];
        let #arg_ident = &__deno_core::_ops::to_str(&mut #scope, &#arg_ident, &mut #arg_temp);
      }
    }
    ...
    Arg::String(Strings::CowByte) => {
      ...
      *needs_isolate = true;
      ...
      gs_quote!(generator_state(scope) => {
        // Trade stack space for potentially non-allocating strings
        let #arg_ident = match __deno_core::_ops::to_cow_one_byte(&mut #scope, &#arg_ident) {
    ...
    Arg::State(RefType::Ref, state) => {
      *needs_opstate = true;
      ...
      quote! {
        ...
        let #arg_ident = __deno_core::_ops::opstate_borrow::<#state>(&#arg_ident);
      }
    }
    Arg::State(RefType::Mut, state) => {
      *needs_opstate = true;
      ...
      quote! {
        ...
        let #arg_ident = __deno_core::_ops::opstate_borrow_mut::<#state>(&mut #arg_ident);
      }
    }
    ...
    Arg::SerdeV8(_class) => {
      *needs_scope = true;
      ...
      quote! {
        let #arg_ident = match __deno_core::_ops::serde_v8_to_rust(&mut #scope, #arg_ident) {
          Ok(t) => t,
          Err(#err) => {
            #throw_exception;
          }
...
pub fn from_arg_option(
  ...
  Ok(quote!(
    let Some(#arg_ident) = __deno_core::_ops::#convert(&#arg_ident) else {
      #exception
    };
...
pub fn from_arg_buffer(
  ...
  let to_v8_slice = if matches!(buffer_mode, BufferMode::Detach) {
    ...
    gs_quote!(generator_state(scope) => { __deno_core::_ops::to_v8_slice_detachable::<#array>(&mut #scope, #arg_ident) })
  } else {
    quote!(__deno_core::_ops::to_v8_slice::<#array>(#arg_ident))
  };
...
pub fn from_arg_arraybuffer(
  ...
  let make_v8slice = quote!({
    #temp = match unsafe { __deno_core::_ops::#to_v8_slice(#arg_ident) } {
      Ok(#arg_ident) => #arg_ident,
      Err(#err) => {
        #throw_exception
      }
...
pub fn return_value_infallible(
  ...
  let result = match ret_type.marker() {
    ArgMarker::ArrayBuffer => {
      gs_quote!(generator_state(result) => (__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::ArrayBufferMarker, _>::from(#result)))
    }
    ArgMarker::Serde => {
      gs_quote!(generator_state(result) => (__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::SerdeMarker, _>::from(#result)))
    }
    ArgMarker::Smi => {
      gs_quote!(generator_state(result) => (__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::SmiMarker, _>::from(#result)))
    }
    ArgMarker::Number => {
      gs_quote!(generator_state(result) => (__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::NumberMarker, _>::from(#result)))
    }
    ArgMarker::None => gs_quote!(generator_state(result) => (#result)),
  };
  let res = match ret_type.slow_retval() {
    ArgSlowRetval::RetVal => {
      gs_quote!(generator_state(retval) => (__deno_core::_ops::RustToV8RetVal::to_v8_rv(#result, &mut #retval)))
    }
    ArgSlowRetval::RetValFallible => {
      ...
      gs_quote!(generator_state(scope, retval) => (match __deno_core::_ops::RustToV8Fallible::to_v8_fallible(#result, &mut #scope) {
        Ok(v) => #retval.set(v),
        Err(#err) => {
          #throw_exception
        },
    ...
    ArgSlowRetval::V8Local => {
      generator_state.needs_scope = true;
      gs_quote!(generator_state(scope, retval) => (#retval.set(__deno_core::_ops::RustToV8::to_v8(#result, &mut #scope))))
    }
    ArgSlowRetval::V8LocalNoScope => {
      gs_quote!(generator_state(retval) => (#retval.set(__deno_core::_ops::RustToV8NoScope::to_v8(#result))))
    }
...
pub fn return_value_v8_value(
  ...
  let result = match ret_type.marker() {
    ArgMarker::ArrayBuffer => {
      quote!(__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::ArrayBufferMarker, _>::from(#result))
    }
    ArgMarker::Serde => {
      quote!(__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::SerdeMarker, _>::from(#result))
    }
    ArgMarker::Smi => {
      quote!(__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::SmiMarker, _>::from(#result))
    }
    ArgMarker::Number => {
      quote!(__deno_core::_ops::RustToV8Marker::<__deno_core::_ops::NumberMarker, _>::from(#result))
    }
    ArgMarker::None => quote!(#result),
  };
  let res = match ret_type.slow_retval() {
    ArgSlowRetval::RetVal | ArgSlowRetval::V8Local => {
      quote!(Ok(__deno_core::_ops::RustToV8::to_v8(#result, #scope)))
    }
    ArgSlowRetval::V8LocalNoScope => {
      quote!(Ok(__deno_core::_ops::RustToV8NoScope::to_v8(#result)))
    }
    ArgSlowRetval::RetValFallible | ArgSlowRetval::V8LocalFalliable => {
      quote!(__deno_core::_ops::RustToV8Fallible::to_v8_fallible(#result, #scope))
    }
...
pub(crate) fn throw_exception(
  ...
  gs_quote!(generator_state(scope, opctx) => {
    ...
    #maybe_opctx
    let err = err.into();
    let exception = __deno_core::error::to_v8_error(
      &mut #scope,
...
fn throw_type_error(
  ...
  Ok(gs_quote!(generator_state(scope) => {
    #maybe_scope
    let msg = __deno_core::v8::String::new_from_one_byte(&mut #scope, #message.as_bytes(), __deno_core::v8::NewStringType::Normal).unwrap();
    let exc = __deno_core::v8::Exception::type_error(&mut #scope, msg);
...
fn throw_type_error_string(
  ...
  Ok(gs_quote!(generator_state(scope) => {
    ...
    let msg = __deno_core::v8::String::new(&mut #scope, &format!("{}", __deno_core::anyhow::Error::from(#message))).unwrap();
    let exc = __deno_core::v8::Exception::type_error(&mut #scope, msg);
    #scope.throw_exception(exc);
    return;
  }))
