...
pub fn v8_intermediate_to_global_arg(
  ...
  let arg = match arg {
    Arg::V8Global(_) => quote!(__deno_core::v8::Global::new(&mut #isolate, #i)),
    Arg::OptionV8Global(_) => {
      quote!(__deno_core::v8::Global::new(&mut #isolate, #i))
    }
...
pub fn v8_to_arg(
  ...
  Ok(quote! {
    let Ok(mut #arg_ident) = __deno_core::_ops::#try_convert::<__deno_core::v8::#v8>(#arg_ident) else {
...
pub fn v8slice_to_buffer(
  ...
  let make_arg = match buffer {
    ...
    BufferType::JsBuffer => {
      quote!(let #arg_ident = __deno_core::serde_v8::JsBuffer::from_parts(#v8slice);)
    }
...
pub fn fast_api_typed_array_to_buffer(
  ...
  Ok(quote! {
    ...
    // fastcall
    let #input = unsafe { __deno_core::v8::fast_api::FastApiTypedArray::get_storage_from_pointer_if_aligned(#input) }.expect("Invalid buffer");
    #convert
  })
}
