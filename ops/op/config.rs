...
impl MacroConfig {
  ...
  pub fn from_flags(
    ...
    if flags != flags_sorted {
      return Err(OpError::ImproperlySortedAttribute(flags_sorted.join(", ")));
    }
    ...
    for flag in flags {
      if flag == "fast" {
        ...
        .map_err(|_| OpError::PatternMatchFailed("attribute", flag))?
        .collect::<Vec<_>>();
      } else if flag == "nofast" {
        config.nofast = true;
      } else if flag == "async" {
        ...
        return Err(OpError::InvalidAttribute(flag));
      }
    }

    // Test for invalid attribute combinations
    if config.fast && config.nofast {
      return Err(OpError::InvalidAttributeCombination("fast", "nofast"));
    }
    if config.fast && !config.fast_alternatives.is_empty() {
      return Err(OpError::InvalidAttributeCombination("fast", "fast(...)"));
    }
    if config.fast_alternatives.len() > 1 {
      return Err(OpError::TooManyFastAlternatives);
    }
    if config.fast
      && (config.r#async && !config.async_lazy && !config.async_deferred)
    {
      return Err(OpError::InvalidAttributeCombination("fast", "async"));
    }
    if config.nofast
      && (config.r#async && !config.async_lazy && !config.async_deferred)
    {
      return Err(OpError::InvalidAttributeCombination("nofast", "async"));
    }
    if config.no_side_effects
      && (config.r#async && !config.async_lazy && !config.async_deferred)
    {
      return Err(OpError::InvalidAttributeCombination(
        "no_side_effects",
        "async",
      ));
    }
  ...
  /// Parses the attribute parameters that `proc_macro` passes to the macro. For `#[op(arg1, arg2)]`, the
  /// tokens will be `arg1, arg2`.
  pub fn from_tokens(tokens: TokenStream) -> Result<Self, OpError> {
    ...
    .map_err(|_| OpError::PatternMatchFailed("attribute", attr_string))??;
    Ok(config)
  }

  #[cfg(test)]
  fn from_token_tree(tokens: TokenTree) -> Result<Self, OpError> {
    let attr_string = tokens.to_string();
  ...
  /// If the attribute matches #[op(...)] or #[op], returns `Some(MacroConfig)`, otherwise returns `None`.
  #[cfg(test)]
...
mod tests {
  ...
  fn test_parse(s: &str, expected: MacroConfig) {
    let item_fn = syn::parse_str::<ItemFn>(&format!("#[op{s}] fn x() {{ }}"))
