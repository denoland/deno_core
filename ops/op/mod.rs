...
/// Generate the op macro expansion.
pub(crate) fn op(
  attr: TokenStream,
  item: TokenStream,
...
fn generate_op(
  ...
  match (is_async, config.r#async) {
    (true, false) => return Err(OpError::ShouldBeAsync),
    (false, true) => return Err(OpError::ShouldNotBeAsync),
    _ => {}
  }
  ...
  let (fast_definition, fast_definition_metrics, fast_fn) =
    match generate_dispatch_fast(
      ...
      None => {
        ...
        if config.nofast {
          return Err(OpError::ShouldNotBeFast("nofast"));
        }
        (quote!(None), quote!(None), quote!())
      }
    };
  ...
  Ok(quote! {
    ...
    #vis const fn #name <#(#generic : #bound),*> () -> ::deno_core::_ops::OpDecl {
      ...
      impl <#(#generic : #bound),*> ::deno_core::_ops::Op for #name <#(#generic),*> {
        ...
        const DECL: ::deno_core::_ops::OpDecl = ::deno_core::_ops::OpDecl::new_internal_op(
...
mod tests {
  ...
  #[testing_macros::fixture("op/test_cases/async/*.rs")]
...
deno_ops_compile_test_runner::prelude!();";
    ...
    for item in file.items {
      if let Item::Fn(mut func) = item {
        ...
        let tokens =
          generate_op(config.unwrap(), func).expect("Failed to generate op");
        println!("======== Raw tokens ========:\n{}", tokens.clone());
  ...
  fn test_valid_args_md() {
    ...
    let separator = "\n<!-- START ARGS -->\n";
    let end_separator = "\n<!-- END ARGS -->\n";
    let readme = std::fs::read_to_string("op/README.md").unwrap();
    ...
    if update_expected {
      std::fs::write("op/README.md", actual)
        .expect("Failed to write expectation file");
    } else {
      assert_eq!(
        readme, actual,
